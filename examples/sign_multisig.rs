...
use bellscoin::blockdata::witness::Witness;
use bellscoin::{absolute, ecdsa, secp256k1, Sequence};
...
fn main() {
    ...
    let descriptor = miniscript::Descriptor::<bellscoin::PublicKey>::from_str(&s).unwrap();
    ...
    // `bellscoin::PublicKey` from the `MiniscriptKey` which can be supplied by
    ...
    let mut sigs = HashMap::<bellscoin::PublicKey, ecdsa::Signature>::new();
...
fn spending_transaction() -> bellscoin::Transaction {
    bellscoin::Transaction {
        version: 2,
        lock_time: absolute::LockTime::ZERO,
        input: vec![bellscoin::TxIn {
            previous_output: Default::default(),
            script_sig: bellscoin::ScriptBuf::new(),
            sequence: Sequence::MAX,
            witness: Witness::default(),
        }],
        output: vec![bellscoin::TxOut {
            script_pubkey: bellscoin::ScriptBuf::new(),
            value: 100_000_000,
        }],
    }
}

fn list_of_three_arbitrary_public_keys() -> Vec<bellscoin::PublicKey> {
    #[cfg_attr(feature="cargo-fmt", rustfmt_skip)]
    vec![
        bellscoin::PublicKey::from_slice(&[2; 33]).expect("key 1"),
        bellscoin::PublicKey::from_slice(&[
            0x02,

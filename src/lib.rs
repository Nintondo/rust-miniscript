...
//! use std::str::FromStr;
//!
//! let desc = miniscript::Descriptor::<bellscoin::PublicKey>::from_str("\
//!     sh(wsh(or_d(\
...
//! assert_eq!(
//!     desc.address(bellscoin::Network::Bitcoin).unwrap().to_string(),
...
use bellscoin::locktime::absolute;
...
pub trait MiniscriptKey: Clone + Eq + Ord + fmt::Debug + fmt::Display + hash::Hash {
    ...
    /// The associated [`bellscoin::hashes::sha256::Hash`] for this [`MiniscriptKey`], used in the
    /// sha256 fragment.
    ...
    /// The associated [`bellscoin::hashes::ripemd160::Hash`] for this [`MiniscriptKey`] type, used
    /// in the ripemd160 fragment.
    type Ripemd160: Clone + Eq + Ord + fmt::Display + fmt::Debug + hash::Hash;

    /// The associated [`bellscoin::hashes::hash160::Hash`] for this [`MiniscriptKey`] type, used in
    /// the hash160 fragment.
    type Hash160: Clone + Eq + Ord + fmt::Display + fmt::Debug + hash::Hash;
}

impl MiniscriptKey for bellscoin::secp256k1::PublicKey {
...
impl MiniscriptKey for bellscoin::PublicKey {
...
impl MiniscriptKey for bellscoin::secp256k1::XOnlyPublicKey {
    type Sha256 = sha256::Hash;
    type Hash256 = hash256::Hash;
    type Ripemd160 = ripemd160::Hash;
    type Hash160 = hash160::Hash;
...
pub trait ToPublicKey: MiniscriptKey {
    ...
    fn to_public_key(&self) -> bellscoin::PublicKey;

    /// Convert an object to x-only pubkey
    fn to_x_only_pubkey(&self) -> bellscoin::secp256k1::XOnlyPublicKey {
        let pk = self.to_public_key();
        bellscoin::secp256k1::XOnlyPublicKey::from(pk.inner)
    }
...
impl ToPublicKey for bellscoin::PublicKey {
    fn to_public_key(&self) -> bellscoin::PublicKey {
        *self
    }
...
impl ToPublicKey for bellscoin::secp256k1::PublicKey {
    fn to_public_key(&self) -> bellscoin::PublicKey {
        bellscoin::PublicKey::new(*self)
    }
...
impl ToPublicKey for bellscoin::secp256k1::XOnlyPublicKey {
    fn to_public_key(&self) -> bellscoin::PublicKey {
        ...
        bellscoin::PublicKey::from_slice(&data)
...
pub enum Error {
    ...
    AddrError(bellscoin::address::Error),
    ...
    Secp(bellscoin::secp256k1::Error),
...
#[doc(hidden)]
impl From<bellscoin::secp256k1::Error> for Error {
    fn from(e: bellscoin::secp256k1::Error) -> Error {
        Error::Secp(e)
    }
}
...
impl From<bellscoin::address::Error> for Error {
    fn from(e: bellscoin::address::Error) -> Error {
        Error::AddrError(e)
    }
}
...
#[cfg(test)]
fn hex_script(s: &str) -> bellscoin::ScriptBuf {
    let v: Vec<u8> = bellscoin::hashes::hex::FromHex::from_hex(s).unwrap();
    bellscoin::ScriptBuf::from(v)
}

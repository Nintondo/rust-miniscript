...
pub mod test {
    use bellscoin;
    use bellscoin::hashes::{hash160, ripemd160, sha256, sha256d, Hash};
    ...
    pub type TestData = (
        Miniscript<bellscoin::PublicKey, Segwitv0>,
    ...
    pub fn gen_bitcoin_pubkeys(n: usize, compressed: bool) -> Vec<bellscoin::PublicKey> {
        gen_secp_pubkeys(n)
            .into_iter()
            .map(|inner| bellscoin::PublicKey { inner, compressed })
            .collect()
    }
    ...
    fn find_keys() {
        gen_testcases().into_iter().for_each(|(ms, k, _, _)| {
            assert_eq!(ms.iter_pk().collect::<Vec<bellscoin::PublicKey>>(), k);
        })
    }
}

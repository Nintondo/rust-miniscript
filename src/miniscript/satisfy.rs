...
use bellscoin::hashes::hash160;
use bellscoin::key::XOnlyPublicKey;
use bellscoin::taproot::{ControlBlock, LeafVersion, TapLeafHash};
use bellscoin::{absolute, Sequence};
use sync::Arc;
...
pub trait Satisfier<Pk: MiniscriptKey + ToPublicKey> {
    ...
    fn lookup_ecdsa_sig(&self, _: &Pk) -> Option<bellscoin::ecdsa::Signature> {
        None
    }
    ...
    /// Given a raw `Pkh`, lookup corresponding [`bellscoin::PublicKey`]
    fn lookup_raw_pkh_pk(&self, _: &hash160::Hash) -> Option<bellscoin::PublicKey> {
        None
    }

    /// Given a raw `Pkh`, lookup corresponding [`bellscoin::secp256k1::XOnlyPublicKey`]
    ...
    ) -> Option<(XOnlyPublicKey, bellscoin::taproot::Signature)> {
        None
    }
...
impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk> for HashMap<Pk, bellscoin::ecdsa::Signature> {
    fn lookup_ecdsa_sig(&self, key: &Pk) -> Option<bellscoin::ecdsa::Signature> {
        self.get(key).copied()
    }
}

impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk>
    for HashMap<(Pk, TapLeafHash), bellscoin::taproot::Signature>
{
    fn lookup_tap_leaf_script_sig(
        &self,
        key: &Pk,
...
impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk>
    for HashMap<hash160::Hash, (Pk, bellscoin::ecdsa::Signature)>
where
    Pk: MiniscriptKey + ToPublicKey,
{
    ...
    fn lookup_raw_pkh_pk(&self, pk_hash: &hash160::Hash) -> Option<bellscoin::PublicKey> {
...
impl<Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk>
    for HashMap<(hash160::Hash, TapLeafHash), (Pk, bellscoin::taproot::Signature)>
where
    Pk: MiniscriptKey + ToPublicKey,
{
    fn lookup_tap_leaf_script_sig(
...
impl<'a, Pk: MiniscriptKey + ToPublicKey, S: Satisfier<Pk>> Satisfier<Pk> for &'a S {
    fn lookup_ecdsa_sig(&self, p: &Pk) -> Option<bellscoin::ecdsa::Signature> {
...
macro_rules! impl_tuple_satisfier {
    ($($ty:ident),*) => {
            ...
            $($ty: Satisfier< Pk>,)*
        {
            ...
            fn lookup_tap_leaf_script_sig(&self, key: &Pk, h: &TapLeafHash) -> Option<bellscoin::taproot::Signature> {
                let &($(ref $ty,)*) = self;
                $(

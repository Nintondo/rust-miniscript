...
use bellscoin::hashes::{hash160, ripemd160, sha256};
use bellscoin::{secp256k1, Address, Network, Script, ScriptBuf, TxIn, Witness};
use sync::Arc;
...
impl Descriptor<DescriptorPublicKey> {
    ...
    /// Convert all the public keys in the descriptor to [`bellscoin::PublicKey`] by deriving them or
    /// otherwise converting them. All [`bellscoin::secp256k1::XOnlyPublicKey`]s are converted to by adding a
    /// default(0x02) y-coordinate.
    ///
    /// This is a shorthand for:
    ///
    /// ```
    /// # use miniscript::{Descriptor, DescriptorPublicKey, bellscoin::secp256k1::Secp256k1};
    ...
    pub fn derived_descriptor<C: secp256k1::Verification>(
        ...
        index: u32,
    ) -> Result<Descriptor<bellscoin::PublicKey>, ConversionError> {
    ...
    pub fn find_derivation_index_for_spk<C: secp256k1::Verification>(
        ...
        range: Range<u32>,
    ) -> Result<Option<(u32, Descriptor<bellscoin::PublicKey>)>, ConversionError> {
...
impl Descriptor<DefiniteDescriptorKey> {
    ...
    /// use miniscript::bellscoin::secp256k1;
    /// use std::str::FromStr;
    ///
    /// // test from bip 86
    ...
    pub fn derived_descriptor<C: secp256k1::Verification>(
        ...
        impl<'a, C: secp256k1::Verification>
            Translator<DefiniteDescriptorKey, bellscoin::PublicKey, ConversionError>
            for Derivator<'a, C>
        {
            fn pk(
                &mut self,
                pk: &DefiniteDescriptorKey,
            ) -> Result<bellscoin::PublicKey, ConversionError> {
                pk.derive_public_key(self.0)
            }

            translate_hash_clone!(DefiniteDescriptorKey, bellscoin::PublicKey, ConversionError);
        }
...
mod tests {
    ...
    use bellscoin::blockdata::opcodes::all::{OP_CLTV, OP_CSV};
    use bellscoin::blockdata::script::Instruction;
    use bellscoin::blockdata::{opcodes, script};
    use bellscoin::hashes::hex::FromHex;
    use bellscoin::hashes::{hash160, sha256, Hash};
    use bellscoin::script::PushBytes;
    use bellscoin::sighash::EcdsaSighashType;
    use bellscoin::{self, bip32, secp256k1, PublicKey, Sequence};
    ...
    pub fn script_pubkey() {
        ...
        assert_eq!(
            bare.address(Network::Bellscoin).unwrap_err().to_string(),
        ...
        assert_eq!(
            pkh.address(Network::Bellscoin,).unwrap().to_string(),
        ...
        assert_eq!(
            wpkh.address(Network::Bellscoin,).unwrap().to_string(),
        ...
        assert_eq!(
            shwpkh.address(Network::Bellscoin,).unwrap().to_string(),
        ...
        assert_eq!(
            sh.address(Network::Bellscoin,).unwrap().to_string(),
        ...
        assert_eq!(
            wsh.address(Network::Bellscoin,).unwrap().to_string(),
        ...
        assert_eq!(
            shwsh.address(Network::Bellscoin,).unwrap().to_string(),
    ...
    fn satisfy() {
        ...
        let pk = bellscoin::PublicKey::new(secp256k1::PublicKey::from_secret_key(&secp, &sk));
        ...
        struct SimpleSat {
            ...
            pk: bellscoin::PublicKey,
        }

        impl Satisfier<bellscoin::PublicKey> for SimpleSat {
            fn lookup_ecdsa_sig(
                &self,
                pk: &bellscoin::PublicKey,
            ) -> Option<bellscoin::ecdsa::Signature> {
                if *pk == self.pk {
                    Some(bellscoin::ecdsa::Signature {
                        sig: self.sig,
                        hash_ty: bellscoin::sighash::EcdsaSighashType::All,
                    })
                } else {
                    None
                }
        ...
        let mut txin = bellscoin::TxIn {
            previous_output: bellscoin::OutPoint::default(),
            script_sig: bellscoin::ScriptBuf::new(),
        ...
        assert_eq!(bare.unsigned_script_sig(), bellscoin::ScriptBuf::new());
        ...
        assert_eq!(pkh.unsigned_script_sig(), bellscoin::ScriptBuf::new());
        ...
        assert_eq!(wpkh.unsigned_script_sig(), bellscoin::ScriptBuf::new());
        ...
        assert_eq!(sh.unsigned_script_sig(), bellscoin::ScriptBuf::new());
        ...
        assert_eq!(wsh.unsigned_script_sig(), bellscoin::ScriptBuf::new());
    ...
    #[test]
    fn after_is_cltv() {
        let descriptor = Descriptor::<bellscoin::PublicKey>::from_str("wsh(after(1000))").unwrap();
    ...
    #[test]
    fn older_is_csv() {
        let descriptor = Descriptor::<bellscoin::PublicKey>::from_str("wsh(older(1000))").unwrap();
        let script = descriptor.explicit_script().unwrap();
    ...
    #[test]
    fn tr_script_pubkey() {
        let key = Descriptor::<bellscoin::PublicKey>::from_str(
    ...
    #[test]
    fn roundtrip_tests() {
        let descriptor = Descriptor::<bellscoin::PublicKey>::from_str("multi");
        assert_eq!(
    ...
    #[test]
    fn empty_thresh() {
        let descriptor = Descriptor::<bellscoin::PublicKey>::from_str("thresh");
        assert_eq!(
            descriptor.unwrap_err().to_string(),
            "unexpected «no arguments given»"
        )
    ...
    fn witness_stack_for_andv_is_arranged_in_correct_order() {
        // arrange
        let a = bellscoin::PublicKey::from_str(
        ...
        let b = bellscoin::PublicKey::from_str(
        ...
        let descriptor = Descriptor::<bellscoin::PublicKey>::from_str(&format!(
    ...
    fn parse_descriptor_key() {
        ...
        let expected = DescriptorPublicKey::Single(SinglePub {
            key: SinglePubKey::FullKey(bellscoin::PublicKey::from_str(
    ...
    fn test_sortedmulti() {
        fn _test_sortedmulti(raw_desc_one: &str, raw_desc_two: &str, raw_addr_expected: &str) {
            ...
            let addr_one = desc_one
                ...
                .unwrap()
                .address(bellscoin::Network::Bellscoin)
                .unwrap();
            let addr_two = desc_two
                ...
                .address(bellscoin::Network::Bellscoin)
                .unwrap();
            let addr_expected = bellscoin::Address::from_str(raw_addr_expected)
                .unwrap()
                .assume_checked();
...
pk(03f28773c2d975288bc7d1d205c3748651b075fbc6610e58cddeeddf8f19405aa8))";
    ...
    #[test]
    fn test_context_pks() {
        let comp_key = bellscoin::PublicKey::from_str(
            "02015e4cb53458bf813db8c79968e76e10d13ed6426a23fa71c2f41ba021c2a7ab",
        )
        .unwrap();
        let x_only_key = bellscoin::key::XOnlyPublicKey::from_str(
            "015e4cb53458bf813db8c79968e76e10d13ed6426a23fa71c2f41ba021c2a7ab",
        )
        .unwrap();
        let uncomp_key = bellscoin::PublicKey::from_str("04015e4cb53458bf813db8c79968e76e10d13ed6426a23fa71c2f41ba021c2a7ab0d46021e9e69ef061eb25eab41ae206187b2b05e829559df59d78319bd9267b4").unwrap();

...
use bellscoin::hashes::{hash160, sha256d, Hash};
use bellscoin::psbt::{self, Psbt};
use bellscoin::secp256k1::{self, Secp256k1, VerifyOnly};
use bellscoin::sighash::{self, SighashCache};
use bellscoin::taproot::{self, ControlBlock, LeafVersion, TapLeafHash};
use bellscoin::{absolute, bip32, Script, ScriptBuf, Sequence};
...
pub enum InputError {
    ...
    SecpErr(bellscoin::secp256k1::Error),
    /// Key errors
    KeyErr(bellscoin::key::Error),
...
#[doc(hidden)]
impl From<bellscoin::secp256k1::Error> for InputError {
    fn from(e: bellscoin::secp256k1::Error) -> InputError {
        InputError::SecpErr(e)
    }
}

#[doc(hidden)]
impl From<bellscoin::key::Error> for InputError {
    fn from(e: bellscoin::key::Error) -> InputError {
        InputError::KeyErr(e)
    }
}
...
impl<'psbt, Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk> for PsbtInputSatisfier<'psbt> {
    fn lookup_tap_key_spend_sig(&self) -> Option<bellscoin::taproot::Signature> {
    ...
    fn lookup_tap_leaf_script_sig(
        ...
        pk: &Pk,
        lh: &TapLeafHash,
    ) -> Option<bellscoin::taproot::Signature> {
    ...
    fn lookup_raw_pkh_pk(&self, pkh: &hash160::Hash) -> Option<bellscoin::PublicKey> {
        self.psbt.inputs[self.index]
            .bip32_derivation
            .iter()
            .find(|&(pubkey, _)| pubkey.to_pubkeyhash(SigType::Ecdsa) == *pkh)
            .map(|(pubkey, _)| bellscoin::PublicKey::new(*pubkey))
    }

    fn lookup_tap_control_block_map(
        &self,
    ) -> Option<&BTreeMap<ControlBlock, (bellscoin::ScriptBuf, LeafVersion)>> {
    ...
    fn lookup_raw_pkh_tap_leaf_script_sig(
        ...
        bellscoin::secp256k1::XOnlyPublicKey,
        bellscoin::taproot::Signature,
    )> {
        self.psbt.inputs[self.index]
            .tap_script_sigs
            .iter()
    ...
    fn lookup_ecdsa_sig(&self, pk: &Pk) -> Option<bellscoin::ecdsa::Signature> {
        self.psbt.inputs[self.index]
            .partial_sigs
    ...
    fn lookup_raw_pkh_ecdsa_sig(
        &self,
        pkh: &hash160::Hash,
    ) -> Option<(bellscoin::PublicKey, bellscoin::ecdsa::Signature)> {
        self.psbt.inputs[self.index]
            .partial_sigs
            .iter()
...
pub trait PsbtExt {
    ...
    /// and outputs a extracted bellscoin::Transaction
    ...
    fn extract<C: secp256k1::Verification>(
        &self,
        secp: &Secp256k1<C>,
    ) -> Result<bellscoin::Transaction, Error>;
    ...
    /// the [`sighash::TapSighashType::Default`](bellscoin::sighash::TapSighashType::Default) is chosen
    /// for for taproot spends, otherwise [`EcdsaSighashType::All`](bellscoin::sighash::EcdsaSighashType::All) is chosen.
    ...
    ///
    /// [`SighashCache`]: bellscoin::sighash::SighashCache
    fn sighash_msg<T: Borrow<bellscoin::Transaction>>(
        &self,
        idx: usize,
...
impl PsbtExt for Psbt {
    ...
    fn sighash_msg<T: Borrow<bellscoin::Transaction>>(
        ...
        let prevouts = bellscoin::sighash::Prevouts::All(&prevouts);
        let inp_spk =
...
pub trait PsbtInputExt {
    ...
    fn update_with_descriptor_unchecked(
        &mut self,
        descriptor: &Descriptor<DefiniteDescriptorKey>,
    ) -> Result<Descriptor<bellscoin::PublicKey>, descriptor::ConversionError>;
}
...
impl Translator<DefiniteDescriptorKey, bellscoin::PublicKey, descriptor::ConversionError>
    for KeySourceLookUp
{
    fn pk(
        &mut self,
        xpk: &DefiniteDescriptorKey,
    ) -> Result<bellscoin::PublicKey, descriptor::ConversionError> {
...
trait PsbtFields {
    ...
    fn tap_internal_key(&mut self) -> &mut Option<bellscoin::key::XOnlyPublicKey>;
    fn tap_key_origins(
        &mut self,
    ) -> &mut BTreeMap<bellscoin::key::XOnlyPublicKey, (Vec<TapLeafHash>, bip32::KeySource)>;
...
fn update_item_with_descriptor_helper<F: PsbtFields>(
    ...
    // failed.
) -> Result<(Descriptor<bellscoin::PublicKey>, bool), descriptor::ConversionError> {
...
mod tests {
    ...
    use bellscoin::bip32::{DerivationPath, ExtendedPubKey};
    use bellscoin::consensus::encode::deserialize;
    use bellscoin::hashes::hex::FromHex;
    use bellscoin::key::XOnlyPublicKey;
    use bellscoin::secp256k1::PublicKey;
    use bellscoin::{absolute, OutPoint, TxIn, TxOut};
    ...
    fn test_extract_bip174() {
        let psbt = bellscoin::psbt::PartiallySignedTransaction::deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
        let secp = Secp256k1::verification_only();
        let tx = psbt.extract(&secp).unwrap();
        let expected: bellscoin::Transaction = deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
    ...
    fn test_update_item_non_tr_multi() {
...

        {
            ...
            let derived = Descriptor::<bellscoin::PublicKey>::from_str(&derived).unwrap();
    ...
    fn test_update_input_checks() {
        ...
        let mut non_witness_utxo = bellscoin::Transaction {
            version: 1,

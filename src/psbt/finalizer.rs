...
use bellscoin::sighash::Prevouts;
use bellscoin::taproot::LeafVersion;
use bellscoin::{PublicKey, Script, ScriptBuf, TxOut, Witness};
...
pub(super) fn get_utxo(psbt: &Psbt, index: usize) -> Result<&bellscoin::TxOut, InputError> {
...
pub(super) fn prevouts(psbt: &Psbt) -> Result<Vec<&bellscoin::TxOut>, super::Error> {
    let mut utxos = vec![];
...
fn get_descriptor(psbt: &Psbt, index: usize) -> Result<Descriptor<PublicKey>, InputError> {
    ...
    if script_pubkey.is_p2pk() {
        ...
        match bellscoin::PublicKey::from_slice(&pk_bytes[1..script_pubkey_len - 1]) {
        ...
        let partial_sig_contains_pk = inp.partial_sigs.iter().find(|&(&pk, _sig)| {
            ...
            let addr = bellscoin::Address::p2pkh(&pk, bellscoin::Network::Bellscoin);
        ...
        let partial_sig_contains_pk = inp.partial_sigs.iter().find(|&(&pk, _sig)| {
            // Indirect way to check the equivalence of pubkey-hashes.
            // Create a pubkey hash and check if they are the same.
            let addr = bellscoin::Address::p2wpkh(&pk, bellscoin::Network::Bellscoin)
        ...
        if let Some(ref witness_script) = inp.witness_script {
            ...
            let ms = Miniscript::<bellscoin::PublicKey, Segwitv0>::parse_with_ext(
                witness_script,
                &ExtParams::allow_all(),
            )?;
            ...
            Some(ref redeem_script) => {
                ...
                if redeem_script.is_v0_p2wsh() {
                    ...
                    if let Some(ref redeem_script) = inp.redeem_script {
                        let ms = Miniscript::<bellscoin::PublicKey, Legacy>::parse_with_ext(
                            redeem_script,
                            &ExtParams::allow_all(),
                        )?;
        ...
        let ms = Miniscript::<bellscoin::PublicKey, BareCtx>::parse_with_ext(
            &script_pubkey,
            &ExtParams::allow_all(),
        )?;
...
fn finalize_input_helper<C: secp256k1::Verification>(
    ...
    let witness = bellscoin::Witness::from_slice(&witness);
    let utxos = prevouts(psbt)?;
    let utxos = &Prevouts::All(&utxos);
    interpreter_inp_check(psbt, secp, index, utxos, &witness, &script_sig)?;

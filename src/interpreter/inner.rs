...
use bellscoin::taproot::{ControlBlock, TAPROOT_ANNEX_PREFIX};
use bellscoin::Witness;
...
fn pk_from_slice(slice: &[u8], require_compressed: bool) -> Result<bellscoin::PublicKey, Error> {
    if let Ok(pk) = bellscoin::PublicKey::from_slice(slice) {
...
) -> Result<bellscoin::PublicKey, Error> {
...
fn script_from_stack_elem<Ctx: ScriptContext>(
    ...
    match *elem {
        stack::Element::Push(sl) => {
            Miniscript::parse_with_ext(&bellscoin::Script::from_bytes(sl), &ExtParams::allow_all())
                .map_err(Error::from)
        }
...
pub(super) fn from_txdata<'txin>(
    spk: &bellscoin::Script,
    script_sig: &'txin bellscoin::Script,
    witness: &'txin Witness,
) -> Result<(Inner, Stack<'txin>, Option<bellscoin::ScriptBuf>), Error> {
        ...
        if !wit_stack.is_empty() {
            ...
            match ssig_stack.pop() {
                Some(elem) => {
                    ...
                    if *spk
                        == bellscoin::ScriptBuf::new_p2pkh(&pk.to_pubkeyhash(SigType::Ecdsa).into())
                    {
                        Ok((
        ...
        if !ssig_stack.is_empty() {
            ...
            match wit_stack.pop() {
                Some(elem) => {
                    ...
                    if *spk == bellscoin::ScriptBuf::new_v0_p2wpkh(&hash160.into()) {
                        Ok((
                            Inner::PublicKey(pk.into(), PubkeyType::Wpkh),
                            wit_stack,
                            Some(bellscoin::ScriptBuf::new_p2pkh(&hash160.into())), // bip143, why..
                        ))
                    } else {
        ...
        if !ssig_stack.is_empty() {
            ...
            match wit_stack.pop() {
                Some(elem) => {
                    ...
                    if *spk == bellscoin::ScriptBuf::new_v0_p2wsh(&scripthash.into()) {
                        Ok((
    ...
    // ** pay to taproot **//
        ...
        if !ssig_stack.is_empty() {
            ...
            let output_key = bellscoin::key::XOnlyPublicKey::from_slice(spk[2..].as_bytes())
            ...
            match wit_stack.len() {
                ...
                _ => {
                    ...
                    let secp = bellscoin::secp256k1::Secp256k1::verification_only();
        ...
        match ssig_stack.pop() {
            Some(elem) => {
                if let stack::Element::Push(slice) = elem {
                    ...
                    if *spk != bellscoin::ScriptBuf::new_p2sh(&scripthash.into()) {
                        ...
                        return match wit_stack.pop() {
                            Some(elem) => {
                                if !ssig_stack.is_empty() {
                                    ...
                                    let hash160 = pk.to_pubkeyhash(SigType::Ecdsa);
                                    if slice
                                        == bellscoin::ScriptBuf::new_v0_p2wpkh(&hash160.into())
                                            .as_bytes()
                                    {
                                        Ok((
                        ...
                        return match wit_stack.pop() {
                            Some(elem) => {
                                if !ssig_stack.is_empty() {
                                    ...
                                    let scripthash = sha256::Hash::hash(script.as_bytes());
                                    if slice
                                        == bellscoin::ScriptBuf::new_v0_p2wsh(&scripthash.into())
                                            .as_bytes()
                                    {
                                        Ok((
    ...
    // ** bare script **
        ...
        if wit_stack.is_empty() {
            ...
            let miniscript = Miniscript::<bellscoin::PublicKey, BareCtx>::parse_with_ext(
                spk,
                &ExtParams::allow_all(),
            )?;
            let miniscript = miniscript.to_no_checks_ms();
...
impl<Ctx: ScriptContext> ToNoChecks for Miniscript<bellscoin::PublicKey, Ctx> {
    fn to_no_checks_ms(&self) -> Miniscript<BitcoinKey, NoChecks> {
        struct TranslateFullPk;

        impl Translator<bellscoin::PublicKey, BitcoinKey, ()> for TranslateFullPk {
            fn pk(&mut self, pk: &bellscoin::PublicKey) -> Result<BitcoinKey, ()> {
                Ok(BitcoinKey::Fullkey(*pk))
            }

            translate_hash_clone!(bellscoin::PublicKey, BitcoinKey, ());
        }
...
impl<Ctx: ScriptContext> ToNoChecks for Miniscript<bellscoin::key::XOnlyPublicKey, Ctx> {
    fn to_no_checks_ms(&self) -> Miniscript<BitcoinKey, NoChecks> {
        // specify the () error type as this cannot error
        struct TranslateXOnlyPk;

        impl Translator<bellscoin::key::XOnlyPublicKey, BitcoinKey, ()> for TranslateXOnlyPk {
            fn pk(&mut self, pk: &bellscoin::key::XOnlyPublicKey) -> Result<BitcoinKey, ()> {
                Ok(BitcoinKey::XOnlyPublicKey(*pk))
            }

            translate_hash_clone!(bellscoin::key::XOnlyPublicKey, BitcoinKey, ());
        }
...
mod tests {
    ...
    use bellscoin::blockdata::script;
    use bellscoin::hashes::hex::FromHex;
    use bellscoin::hashes::{hash160, sha256, Hash};
    use bellscoin::script::PushBytes;
    use bellscoin::{self, ScriptBuf};

    use super::*;
    use crate::miniscript::analyzable::ExtParams;

    struct KeyTestData {
        pk_spk: bellscoin::ScriptBuf,
        pk_sig: bellscoin::ScriptBuf,
        pkh_spk: bellscoin::ScriptBuf,
        pkh_sig: bellscoin::ScriptBuf,
        pkh_sig_justkey: bellscoin::ScriptBuf,
        wpkh_spk: bellscoin::ScriptBuf,
        wpkh_stack: Witness,
        wpkh_stack_justkey: Witness,
        sh_wpkh_spk: bellscoin::ScriptBuf,
        sh_wpkh_sig: bellscoin::ScriptBuf,
        sh_wpkh_stack: Witness,
        sh_wpkh_stack_justkey: Witness,
    }

    impl KeyTestData {
        fn from_key(key: bellscoin::PublicKey) -> KeyTestData {
            ...
            let wpkh_spk = bellscoin::ScriptBuf::new_v0_p2wpkh(&wpkhash);
            let wpkh_scripthash = hash160::Hash::hash(wpkh_spk.as_bytes()).into();

            KeyTestData {
                pk_spk: bellscoin::ScriptBuf::new_p2pk(&key),
                pkh_spk: bellscoin::ScriptBuf::new_p2pkh(&pkhash),
                ...
                sh_wpkh_spk: bellscoin::ScriptBuf::new_p2sh(&wpkh_scripthash),
    ...
    struct FixedTestData {
        pk_comp: bellscoin::PublicKey,
        pk_uncomp: bellscoin::PublicKey,
    }

    fn fixed_test_data() -> FixedTestData {
        FixedTestData {
            pk_comp: bellscoin::PublicKey::from_str(
                "\
                025edd5cc23c51e87a497ca815d5dce0f8ab52554f849ed8995de64c5f34ce7143\
            ",
            )
            .unwrap(),
            pk_uncomp: bellscoin::PublicKey::from_str(
                "\
    ...
    fn pubkey_pk() {
        ...
        let blank_script = bellscoin::ScriptBuf::new();
        ...
        spk[1] = 5;
        let spk = bellscoin::ScriptBuf::from(spk);
        let err = from_txdata(&spk, &bellscoin::ScriptBuf::new(), &empty_wit).unwrap_err();
    ...
    fn pubkey_pkh() {
        ...
        let err = from_txdata(&comp.pkh_spk, &bellscoin::ScriptBuf::new(), &empty_wit).unwrap_err();
    ...
    fn ms_inner_script(ms: &str) -> (Miniscript<BitcoinKey, NoChecks>, bellscoin::ScriptBuf) {
        let ms =
            Miniscript::<bellscoin::PublicKey, Segwitv0>::from_str_ext(ms, &ExtParams::insane())
                .unwrap();
        let spk = ms.encode();

...
use bellscoin::{absolute, secp256k1, sighash, taproot, Sequence, TxOut, Witness};
...
pub struct Interpreter<'txin> {
    ...
    script_code: Option<bellscoin::ScriptBuf>,
    age: Sequence,
...
pub enum KeySigPair {
    ...
    Ecdsa(bellscoin::PublicKey, bellscoin::ecdsa::Signature),
    /// A x-only key and corresponding Schnorr signature
    Schnorr(
        bellscoin::key::XOnlyPublicKey,
        bellscoin::taproot::Signature,
    ),
}

impl KeySigPair {
    /// Obtain a pair of ([`bellscoin::PublicKey`], [`bellscoin::ecdsa::Signature`]) from [`KeySigPair`]
    pub fn as_ecdsa(&self) -> Option<(bellscoin::PublicKey, bellscoin::ecdsa::Signature)> {
        match self {
    ...
    /// Obtain a pair of ([`bellscoin::secp256k1::XOnlyPublicKey`], [`bellscoin::taproot::Signature`]) from [`KeySigPair`]
    pub fn as_schnorr(
        &self,
    ) -> Option<(
        bellscoin::key::XOnlyPublicKey,
...
enum BitcoinKey {
    // Full key
    Fullkey(bellscoin::PublicKey),
    // Xonly key
    XOnlyPublicKey(bellscoin::key::XOnlyPublicKey),
}
...
impl From<bellscoin::PublicKey> for BitcoinKey {
    fn from(pk: bellscoin::PublicKey) -> Self {
        BitcoinKey::Fullkey(pk)
    }
}

impl From<bellscoin::key::XOnlyPublicKey> for BitcoinKey {
    fn from(xpk: bellscoin::key::XOnlyPublicKey) -> Self {
...
impl<'txin> Interpreter<'txin> {
    ...
    pub fn verify_sig<C: secp256k1::Verification, T: Borrow<TxOut>>(
        ...
        let mut cache = bellscoin::sighash::SighashCache::new(tx);
        match sig {
    ...
    /// since it cannot distinguish between sorted and unsorted multisigs (and anyway
    /// it can only see the final keys, keyorigin info is lost in serializing to Bitcoin).
    /// x-only keys are translated to [`bellscoin::PublicKey`] with 0x02 prefix.
    pub fn inferred_descriptor(&self) -> Result<Descriptor<bellscoin::PublicKey>, crate::Error> {
...
fn verify_sersig<'txin>(
    ...
    match pk {
        BitcoinKey::Fullkey(pk) => {
            let ecdsa_sig = bellscoin::ecdsa::Signature::from_slice(sigser)?;
        ...
        BitcoinKey::XOnlyPublicKey(x_only_pk) => {
            let schnorr_sig = bellscoin::taproot::Signature::from_slice(sigser)?;
...
mod tests {
    ...
    use bellscoin;
    use bellscoin::hashes::{hash160, ripemd160, sha256, Hash};
    use bellscoin::secp256k1::{self, Secp256k1};
    ...
    fn setup_keys_sigs(
        ...
        Vec<bellscoin::PublicKey>,
        Vec<Vec<u8>>,
        Vec<bellscoin::ecdsa::Signature>,
        secp256k1::Message,
        Secp256k1<secp256k1::All>,
        Vec<bellscoin::key::XOnlyPublicKey>,
        Vec<bellscoin::taproot::Signature>,
        Vec<Vec<u8>>,
    ) {
        ...
        for i in 1..n + 1 {
            ...
            let pk = bellscoin::PublicKey {
                inner: secp256k1::PublicKey::from_secret_key(&secp, &sk),
                compressed: true,
            };
            let sig = secp.sign_ecdsa(&msg, &sk);
            ecdsa_sigs.push(bellscoin::ecdsa::Signature {
                sig,
            ...
            let keypair = bellscoin::key::KeyPair::from_secret_key(&secp, &sk);
            let (x_only_pk, _parity) = bellscoin::key::XOnlyPublicKey::from_keypair(&keypair);
            x_only_pks.push(x_only_pk);
            let schnorr_sig = secp.sign_schnorr_with_aux_rand(&msg, &keypair, &[0u8; 32]);
            let schnorr_sig = bellscoin::taproot::Signature {
                sig: schnorr_sig,
                hash_ty: bellscoin::sighash::TapSighashType::Default,
            };
    ...
    fn no_checks_ms(ms: &str) -> Miniscript<BitcoinKey, NoChecks> {
        ...
        let elem: Miniscript<bellscoin::PublicKey, NoChecks> =
    ...
    fn x_only_no_checks_ms(ms: &str) -> Miniscript<BitcoinKey, NoChecks> {
        let elem: Miniscript<bellscoin::key::XOnlyPublicKey, NoChecks> =
            Miniscript::from_str_ext(ms, &ExtParams::allow_all()).unwrap();
        elem.to_no_checks_ms()
    }
}

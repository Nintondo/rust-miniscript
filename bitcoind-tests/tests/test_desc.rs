...
use miniscript::bellscoin::{self, ecdsa, taproot, ScriptBuf};
...
pub fn test_desc_satisfy(
    ...
    let desc_address = derived_desc.address(bellscoin::Network::Regtest);
...
fn find_sks_ms<Ctx: ScriptContext>(
    ms: &Miniscript<bellscoin::PublicKey, Ctx>,
    testdata: &TestData,
...
fn find_sk_single_key(pk: bellscoin::PublicKey, testdata: &TestData) -> Vec<secp256k1::SecretKey> {
    let sks = &testdata.secretdata.sks;
    let pks = &testdata.pubdata.pks;
    let i = pks.iter().position(|&x| x.to_public_key() == pk);

...
use bellscoin::psbt::Psbt;
use bellscoin::secp256k1::{self, Secp256k1};
use bellscoin::{psbt, Amount, OutPoint, Sequence, Transaction, TxIn, TxOut, Txid};
use bitcoind::bitcoincore_rpc::{json, Client, RpcApi};
use miniscript::bellscoin::absolute;
...
pub(crate) fn parse_miniscripts(
    ...
    pubdata: &PubData,
) -> Vec<Descriptor<bellscoin::PublicKey>> {
...
pub fn test_from_cpp_ms(cl: &Client, testdata: &TestData) {
    ...
    for wsh in desc_vec.iter() {
        let txid = cl
            .send_to_address(
                &wsh.address(bellscoin::Network::Regtest).unwrap(),
                btc(1),
                None,
                None,
                None,
    ...
    for i in 0..psbts.len() {
        ...
        let mut sighash_cache = bellscoin::sighash::SighashCache::new(&psbts[i].unsigned_tx);
        let sighash_ty = bellscoin::sighash::EcdsaSighashType::All;
